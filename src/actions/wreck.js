...
const correctItem = (items, location) => {
  ...
  return items.find(item => {
    return item.link.includes(`https://www.instagram.com/explore/tags/${location}`);
  });
};
...
export const checkWreck = location => dispatch => {
  ...
  request
    ...
    .then(result => { // picks out the object that contains the pure hashtag
      return correctItem(result.body.items, formattedLocation);
    })
    .then(result => { // takes string with value, converts into number
